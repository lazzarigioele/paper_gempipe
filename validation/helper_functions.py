...
def gapfilling_for_bactabolize(ref_file, file, outfolder, dataset='01_klebsiella'):
    ...
    elif dataset[:2] in ['04', '05', '06']:
        pass   # use default (rich) medium
    ...
    if dataset[:2] in ['01','02','03']:
        ...
        for r in model.reactions: 
            if len(r.metabolites)==1 and list(r.metabolites)[0].id.rsplit('_', 1)[-1]=='e':
                r.bounds = (0, 1000)
        # copy the exchanges of the reference medium:
        for exr_id, bound in dict(ref_model.medium).items():
            if exr_id in [r.id for r in model.reactions]:
                model.reactions.get_by_id(exr_id).lower_bound = -bound
    ...
    print('BEFORE gapfilling', file, 'ref_obj', ref_obj, 'ref_status', ref_status, 'obj', obj, 'status', status)  # log
    
    
    #  determine the minflux according to the dataset:
    if dataset[:2] in ['01','02','03','04','05','06']:
        minflux = 0.6

        
        
    if obj < minflux or status == 'infeasible':

        rids = gempipe.perform_gapfilling(model, ref_model, minflux= minflux, nsol=1, verbose=True)
        ...
        if rids != None:   # at least a solution was found
            for rid in rids: 
                gempipe.import_from_universe(model, ref_model, rid, gpr='')
    ...
    print('AFTER gapfilling', file, 'ref_obj', ref_obj, 'ref_status', ref_status, 'obj', obj, 'status', status)  # log
