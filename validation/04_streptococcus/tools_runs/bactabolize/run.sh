...
cp medium_bactabolize.json /opt/Progs/miniconda3/envs/bactabolize/lib/python3.9/site-packages/bactabolize/data/media_definitions/medium_bactabolize_04.json
...
do {
    ...
    bactabolize draft_model \
        --assembly_fp $f \
        --no_reannotation \
        --ref_proteins_fp iYH543.faa \
        --ref_genes_fp iYH543.fna \
        --ref_model_fp Hirose2024_edited.xml \
        --biomass_reaction_id bio1 \
        --media_type medium_bactabolize_04 \
...
do {
    python << EOF
from helper_functions import *
gapfilling_for_bactabolize('Hirose2024_edited.xml', '$f', 'gapfilled', dataset='04_streptococcus')
EOF
} & done; wait
